...
DeFacto supports **5 languages** in v1:
...
3. On save: `i18n.changeLanguage()`, SecureStore, `profiles.preferred_locale` upsert, RTL applied if switching to/from Arabic.
...
## Phase 4 — Arabic RTL and typography

**Goal:** Arabic is fully usable.

**RTL**

- When `locale === 'ar'`: `I18nManager.allowRTL(true)` + `forceRTL(true)`
- Layout may require an app reload when switching to/from Arabic (documented behavior)
- Prefer `start`/`end` over hardcoded `left`/`right`

**Fonts**

- Load `@expo-google-fonts/noto-sans-arabic`
- When locale is Arabic, swap typography to Noto Sans Arabic

**Testing checklist**

- [ ] Bottom tabs mirror correctly
- [ ] Back buttons and chevrons flip
- [ ] Signup / Profile modals
- [ ] Quiz options and SessionConfigSheet
- [ ] Arabic glyphs render cleanly

**Acceptance:** Arabic layout mirrors in RTL; text renders with Noto font.
...
- Language display names live under `language.en`, `language.ar`, etc. (shown in native script)
...
- Professional native-speaker review (especially Arabic UI copy)
